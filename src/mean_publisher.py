...
def resend_data(msg):
	...
	if count == 10:
		...
		mean_wr.header = msg.wrench.header # will this work?
		dev_wr.header = msg.wrench.header
		# copying force data X
		val = Fx.add(msg.wrench.force.x)
		mean_wr.force.x = val[0]
		dev_wr.force.x = val[1]
		# copying force data Y
		val = Fy.add(msg.wrench.force.y)
		mean_wr.force.y = val[0]
		dev_wr.force.y = val[1]
		# copying force data Z
		val = Fz.add(msg.wrench.force.z)
		mean_wr.force.z = val[0]
		dev_wr.force.z = val[1]
		# copying torque data X
		val = Tx.add(msg.wrench.torque.x)
		mean_wr.torque.x = val[0]
		dev_wr.torque.x = val[1]
		# copying torque data Y
		val = Ty.add(msg.wrench.torque.y)
		mean_wr.torque.y = val[0]
		dev_wr.torque.y = val[1]
		# copying torque data Z
		val = Tz.add(msg.wrench.torque.z)
		mean_wr.torque.z = val[0]
		dev_wr.torque.z = val[1]
		# publishing
